...
# Daniel Yeung
# Section: 204
# Assignment: Canvas Team Program: Cheese Consumed by Llamas Over Time
...
def write_input(x_list, y_list, v):
    ...
    with open('nailed_it.txt', 'w') as datFile:
        datFile.write('\'Group 8\' with Nikolai Steen, Sebastian Warren, Aravindh Diwakar, Daniel Yeung\n')
        datFile.write('November 1, 2023\n')
        datFile.write('\n')
        datFile.write(f'x-values = time, y-values = {v}\n')
        datFile.write('------------------------------------------------------------------\n')
        for i in range(len(x_list)):
            datFile.write(f'{int(x_list[i])}, {int(y_list[i])}\n')
        datFile.write('------------------------------------------------------------------\n')        
...
def print_to_file(x, y, t):
    ...
    with open('nailed_it.txt', 'a') as datFile:
        datFile.write(f'{z:<13} {t}\n')
        # <-- replace this with your code
